...
public interface NotificationDeliveryOutboxMapper extends BaseMapper<NotificationDeliveryOutboxEvent> {
    ...
    @Insert("INSERT INTO notification_delivery_outbox "
            + "(event_id, notification_id, recipient_id, event_type, unread_delta, status, "
            + "attempt_count, max_attempts, create_time, update_time) VALUES "
            + "(#{eventId}, #{notificationId}, #{recipientId}, #{eventType}, #{unreadDelta}, "
    ...
    NotificationDeliveryOutboxEvent selectClaimedEvent(@Param("eventId") String eventId,
                                                       @Param("workerId") String workerId);

    @Select("SELECT event_id FROM notification_delivery_outbox WHERE attempt_count < max_attempts "
            + "AND ((status IN ('pending','failed') AND (next_retry_time IS NULL OR next_retry_time <= NOW())) "
    ...
    @Select("SELECT event_id AS eventId, notification_id AS notificationId, recipient_id AS recipientId, "
            ...
            + "FROM notification_delivery_outbox WHERE status = 'failed' "
