...
@Mapper
public interface UserMusicMonthlySummaryMapper extends BaseMapper<UserMusicMonthlySummary> {

    @Select("SELECT * FROM user_music_monthly_summary " +
            "WHERE user_id = #{userId} AND stat_month BETWEEN #{startMonth} AND #{endMonth} " +
            "AND deleted = 0 ORDER BY stat_month ASC")
    List<UserMusicMonthlySummary> selectByUserAndMonthRange(@Param("userId") Long userId,
                                                             @Param("startMonth") LocalDate startMonth,
                                                             @Param("endMonth") LocalDate endMonth);

    @Select("SELECT MIN(stat_month) AS firstStatMonth, MAX(stat_month) AS lastStatMonth, " +
            "COUNT(*) AS rowCount, COUNT(DISTINCT stat_month) AS coveredMonths, MAX(update_time) AS lastUpdatedAt " +
            "FROM user_music_monthly_summary WHERE deleted = 0")
    Map<String, Object> selectSummaryStatus();

    @Insert("INSERT INTO user_music_monthly_summary (" +
            "user_id, stat_month, play_count, valid_play_count, play_seconds, unique_song_count, active_days, " +
            ...
            "SELECT lh.user_id, #{statMonth}, COUNT(*) AS play_count, " +
            ...
            "COUNT(DISTINCT lh.song_id) AS unique_song_count, COUNT(DISTINCT DATE(lh.create_time)) AS active_days, " +
            ...
            "play_seconds = VALUES(play_seconds), unique_song_count = VALUES(unique_song_count), active_days = VALUES(active_days), " +
    ...
    int refreshMonthlySummary(@Param("statMonth") LocalDate statMonth,
