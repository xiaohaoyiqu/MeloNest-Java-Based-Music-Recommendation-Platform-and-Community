...
public class EmailVerificationServiceImpl implements EmailVerificationService {
    ...
    private static final String CODE_CACHE_PREFIX = "verification:email:code:";
    private static final String SEND_LIMIT_PREFIX = "verification:email:limit:";
    private static final String IP_SEND_LIMIT_PREFIX = "verification:email:ip-limit:";
    private static final SecureRandom RANDOM = new SecureRandom();

    @Override
    public Map<String, Object> sendCode(String email, String scene, String clientIp, String userAgent) {
        String normalizedEmail = normalizeEmail(email);
        String normalizedScene = normalizeScene(scene);
        Integer verificationType = toVerificationType(normalizedScene);

        if (!canSendCode(normalizedEmail, verificationType)) {
            throw new BusinessException(ResultCode.TOO_MANY_REQUESTS, "验证码发送过于频繁，请稍后再试");
        }

        UserVerification active = selectActive(normalizedEmail, verificationType);
        ...
        UserVerification verification = savePendingCode(active, normalizedEmail, verificationType, codeHash, clientIp, userAgent);
        String cacheKey = getCodeCacheKey(normalizedEmail, verificationType);
        redisTemplate.opsForValue().set(cacheKey, codeHash, safeInt(userVerificationConfig.getCodeExpireMinutes(), 5), TimeUnit.MINUTES);

        try {
            EmailSendResult sendResult = emailSender.send(normalizedEmail, normalizedScene, code);
            ...
            invalidateById(verification.getId());
            redisTemplate.delete(cacheKey);
            log.error("发送邮箱验证码失败: email={}, scene={}", DataMaskingUtil.maskEmail(normalizedEmail), normalizedScene);
    ...
    public void verifyCode(String email, String scene, String code) {
        ...
        UserVerification verification = userVerificationMapper.selectValidCode(normalizedEmail, verificationType, now);
    ...
    private UserVerification savePendingCode(UserVerification active, String email, Integer type, String codeHash,
    ...
    private UserVerification selectActive(String email, Integer type) {
        LambdaQueryWrapper<UserVerification> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(UserVerification::getTarget, email)
    ...
    private boolean canSendCode(String email, Integer type) {
        Boolean limited = redisTemplate.hasKey(getSendLimitKey(email, type));
        if (Boolean.TRUE.equals(limited)) {
            return false;
        }
        LocalDateTime oneHourAgo = LocalDateTime.now().minusHours(1);
        Integer count = userVerificationMapper.countRecentCodes(email, type, oneHourAgo);
        return safeInt(count, 0) < safeInt(userVerificationConfig.getMaxSendCountPerHour(), 5);
    }

    private void reserveSendWindow(String email, Integer type, String clientIp) {
        int ttlSeconds = safeInt(userVerificationConfig.getResendIntervalSeconds(), 60);
        String emailLimitKey = getSendLimitKey(email, type);
        String ipLimitKey = getIpSendLimitKey(clientIp, type);

        Boolean emailReserved = redisTemplate.opsForValue().setIfAbsent(emailLimitKey, "1", ttlSeconds, TimeUnit.SECONDS);
        if (!Boolean.TRUE.equals(emailReserved)) {
            throw new BusinessException(ResultCode.TOO_MANY_REQUESTS, "验证码发送过于频繁，请稍后再试");
        }
        ...
        if (!Boolean.TRUE.equals(ipReserved)) {
            redisTemplate.delete(emailLimitKey);
            throw new BusinessException(ResultCode.TOO_MANY_REQUESTS, "验证码发送过于频繁，请稍后再试");
        }
    }
    ...
    private String normalizeScene(String scene) {
        ...
        if ("register".equals(normalized) || "reset".equals(normalized)
                || "login".equals(normalized) || "change_email".equals(normalized)
    ...
    private String normalizeEmail(String email) {
        ...
        if (!DataMaskingUtil.isValidEmail(normalized)) {
    ...
    private String getCodeCacheKey(String email, Integer type) {
        return CODE_CACHE_PREFIX + type + ":" + email;
    }

    private String getSendLimitKey(String email, Integer type) {
        return SEND_LIMIT_PREFIX + type + ":" + email;
    }
